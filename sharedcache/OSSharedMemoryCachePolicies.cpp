...
// #endif
    rc = omrshmem_open(_cache->_cacheLocation, _cache->_configOptions->groupPermissions(), &_cache->_config->_shmhandle, fileName, _cache->_configOptions->cacheSize(), permissions, OMRMEM_CATEGORY_CLASSES, flags, &_cache->_controlFileStatus);
    break;
...
#else
  rc = omrshmem_open(_cache->_cacheLocation, _cache->_configOptions->groupPermissions(), &_cache->_config->_shmhandle, fileName, _cache->_configOptions->cacheSize(), permissions, OMRMEM_CATEGORY_CLASSES_SHC_CACHE, flags, NULL);
...
//#endif
    if (_cache->_configOptions->tryReadOnlyOnOpenFailure()) {//_openMode & J9OSCACHE_OPEN_MODE_TRY_READONLY_ON_FAIL) {
      ...
      rc = omrshmem_open(_cache->_cacheLocation, _cache->_configOptions->groupPermissions(), &_cache->_config->_shmhandle,
			 fileName, _cache->_configOptions->cacheSize(), permissions, OMRMEM_CATEGORY_CLASSES_SHC_CACHE,
