...
OMR::AOTMethodHeader::AOTMethodHeader(uint8_t* compiledCodeStart, uint32_t compiledCodeSize, uint8_t* relocationsStart, uint32_t relocationsSize):
      _compiledCodeStart(compiledCodeStart),
      _compiledCodeSize(compiledCodeSize),
      _relocationsSize(relocationsSize)
      {
      self()->setRelocationsStart(relocationsStart);
      };
...
void
OMR::AOTMethodHeader::setRelocationsStart(uint8_t* reloStart)
   {
   _relocationsStart = reinterpret_cast<TR::RelocationRecordBinaryTemplate*>(reloStart);
   }

TR::RelocationRecordBinaryTemplate*
OMR::AOTMethodHeader::getRelocationsStart()
   {
   return _relocationsStart;
   }
