...
OMR::Z::Peephole::tryToRemoveRedundant32To64BitExtend(bool isSigned)
   ...
   while ((current != NULL) &&
      ...
      {
      TR::InstOpCode curOpCode = current->getOpCode();
      TR::InstOpCode::Mnemonic curOpMnemonic = curOpCode.getMnemonic();
      ...
      if (current->getNumRegisterOperands() > 0 && lgfrReg == current->getRegisterOperand(1))
         ...
         if (replacement != NULL &&
            performTransformation(self()->comp(), "O^O S390 PEEPHOLE: Merging %s [%p] and %s [%p] into %s [%p].\n",
               current->getOpCode().getMnemonicName(), current,
               lgfr->getOpCode().getMnemonicName(), lgfr,
               replacement->getOpCode().getMnemonicName(), replacement))
            {
            auto comp = self()->comp();
            TR::DebugCounter::incStaticDebugCounter(comp,
               TR::DebugCounter::debugCounterName(comp, "z/peephole/%s/%s/%s/(%s)",
