...
void OMR::LocalCSE::examineNode(TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences, TR::Node *parent, int32_t childNum, int32_t *nextLoadIndex, bool *parentCanBeAvailable, int32_t depth)
   ...
   bool alreadyKilledAtOpaqueLoad = false;
   if (hasAliases && !doneCommoning)
      alreadyKilledAtOpaqueLoad = killExpressionsIfOpaqueLoad(node, seenAvailableLoadedSymbolReferences, UseDefAliases);
   ...
   if (node->getOpCode().isStore() || hasAliases)
      ...
      if (hasAliases)
        ...
        if (UseDefAliases.containsAny(tmp, comp()))
           ...
           if (alreadyKilledAtOpaqueLoad) // we want to keep the last load of a particular sym ref alive but no earlier loads should surivive
              seenAvailableLoadedSymbolReferences.set(symRef->getReferenceNumber());
           }
        }
     else
...
bool OMR::LocalCSE::canCommonNodeInVolatilePass(TR::Node *node)
   {
   return node->getOpCode().hasSymbolReference() && (node->getSymbol()->isOpaque() || node->getSymbol()->isAutoOrParm());
   }
...
// This routine kills all prior opaque loads of the same (or aliased) sym ref before we add the current
// opaque load to the available expressions
//
bool OMR::LocalCSE::killExpressionsIfOpaqueLoad(TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences, TR_UseDefAliasSetInterface &UseDefAliases)
   {
   bool isOpaqueRead = false;
   if (!node->getOpCode().isLikeDef() && node->mightHaveOpaqueSymbolReference())
      isOpaqueRead = true;
   ...
   if (isOpaqueRead)
      ...
      if (_volatileState != VOLATILE_ONLY)
         ...
         while (bvi.hasMoreElements())
            ...
            if (comp()->getSymRefTab()->getSymRef(c)->maybeOpaque())
