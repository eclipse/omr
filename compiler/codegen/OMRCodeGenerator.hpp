...
class OMR_EXTENSIBLE CodeGenerator
   ...
   // NOT USED?
   bool supportsDebugCounters(TR::DebugCounterInjectionPoint injectionPoint){ return injectionPoint == TR::TR_BeforeCodegen; }
   ...
   //
   bool supportsLongRegAllocation() {return false;} 
   ...
   bool processInstruction(TR::Instruction *instr, TR_BitVector **registerUsageInfo, int32_t &blockNum, int32_t &isFence, bool traceIt) {return false;}
   uint32_t isPreservedRegister(int32_t regIndex) { return 0; }
   bool isReturnInstruction(TR::Instruction *instr) { return false; }
   bool isBranchInstruction(TR::Instruction *instr) { return false; }
   int32_t isFenceInstruction(TR::Instruction *instr) { return false; }
   bool isAlignmentInstruction(TR::Instruction *instr) { return false; }
   bool isLabelInstruction(TR::Instruction *instr) { return false; }
   TR::Instruction *splitEdge(TR::Instruction *cursor, bool isFallThrough, bool needsJump, TR::Instruction *newSplitLabel, TR::list<TR::Instruction*> *jmpInstrs, bool firstJump = false) { return NULL; }
   TR::Instruction *splitBlockEntry(TR::Instruction *instr) { return NULL; }
   int32_t computeRegisterSaveDescription(TR_BitVector *regs, bool populateInfo = false) { return 0; }
   void processIncomingParameterUsage(TR_BitVector **registerUsageInfo, int32_t blockNum) { return; }
   void updateSnippetMapWithRSD(TR::Instruction *cur, int32_t rsd) { return; }
   ...
   bool hasTargetAddressSnippets() {return false;}
   ...
   bool doInlineAllocate(TR::Node *node) { TR_ASSERT(0, "unexpected call to OMR::CodeGenerator::doInlineAllocate"); return false; }
   ...
   // IA32 only?
   int32_t getVMThreadGlobalRegisterNumber() {return -1;}
