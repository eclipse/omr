...
renameDump(struct OMRPortLibrary *portLibrary, char *filename, pid_t pid, int signalNumber)
{
	...
	size = sizeof(execPath);
	if (-1 == sysctl(proc, nitems(proc), execPath, &size, NULL, 0)) {
		portLibrary->str_printf(portLibrary, filename, EsMaxPath, "Unable to read kern.proc.pathname");
		return 1;
	}

	portLibrary->str_printf(portLibrary, derivedAbsoluteCorePath, PATH_MAX, "%s.core", execPath);
