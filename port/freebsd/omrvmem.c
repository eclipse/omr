...
omrvmem_get_available_physical_memory(struct OMRPortLibrary *portLibrary, uint64_t *freePhysicalMemorySize)
{
	...
	size_t size;
	uint64_t inactivePages = 0, cachedPages = 0, freePages = 0, result;

	size = sizeof(inactivePages);
	if (sysctlbyname("vm.stats.vm.v_inactive_count", &inactivePages, &size, NULL, 0) < 0) {
		intptr_t sysctlbynameError = (intptr_t)errno;
		Trc_PRT_vmem_get_available_physical_memory_failed("sysctlbyname(\"vm.stats.vm.v_inactive_count\", ...) failed",
								  sysctlbynameError);
		return OMRPORT_ERROR_VMEM_OPFAILED;
	}

	size = sizeof(cachedPages);
	if (sysctlbyname("vm.stats.vm.v_cache_count", &cachedPages, &size, NULL, 0) < 0) {
		intptr_t sysctlbynameError = (intptr_t)errno;
		Trc_PRT_vmem_get_available_physical_memory_failed("sysctlbyname(\"vm.stats.vm.v_cache_count\", ...) failed",
								  sysctlbynameError);
		return OMRPORT_ERROR_VMEM_OPFAILED;
	}
	...
	if (sysctlbyname("vm.stats.vm.v_free_count", &freePages, &size, NULL, 0) < 0) {
		intptr_t sysctlbynameError = (intptr_t)errno;
		Trc_PRT_vmem_get_available_physical_memory_failed("sysctlbyname(\"vm.stats.vm.v_free_count\", ...) failed",
								  sysctlbynameError);
		return OMRPORT_ERROR_VMEM_OPFAILED;
	}

	result = (inactivePages + cachedPages + freePages) * pageSize;
...
{
	int32_t result = OMRPORT_ERROR_VMEM_OPFAILED, pageSize = getpagesize();
	struct kinfo_proc *kproc;
	Trc_PRT_vmem_get_process_memory_enter((int32_t)queryType);

	kproc = kinfo_getproc(getpid());

	if (kproc != NULL) {
		result = 0;
		switch (queryType) {
		case OMRPORT_VMEM_PROCESS_PHYSICAL:
			*memorySize = (uint64_t)(kproc->ki_rssize * pageSize);
			break;
		...
		case OMRPORT_VMEM_PROCESS_VIRTUAL:
			*memorySize = (uint64_t)(kproc->ki_size);
			break;
		default:
		...
		Trc_PRT_vmem_get_process_memory_failed("kinfo_getproc() failed", 0);
		result = OMRPORT_ERROR_VMEM_OPFAILED;
	}
