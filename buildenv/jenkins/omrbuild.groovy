...
buildSpec = (params.BUILDSPEC) ? params.BUILDSPEC : null
cmdLine = (params.ghprbCommentBody) ? params.ghprbCommentBody : null
pullId = (params.ghprbPullId) ? params.ghprbPullId : null

cgroupV1Specs = ["linux_x86"]
cgroupV2Specs = ["linux_x86-64"]
dockerSpecs = ["linux_x86-64"]
        ...
        for (build in SPECS[buildSpec].builds) {
            dir("${build.buildDir}") {
                ...
                switch (SPECS[buildSpec].buildSystem) {
                    case 'cmake':
                        sh "cmake ${build.configureArgs} ${configureAppend} .."
                        break
                    case 'autoconf':
                        sh "make -f run_configure.mk OMRGLUE=./example/glue ${build.configureArgs} ${configureAppend}"
                        break
                    default:
...
def runAllStagesInDocker() {
    def customWorkspace = WORKSPACE - "/${JOB_NAME}" + "/${workspaceName}_docker"
    ws(customWorkspace) {
        try {
...
def parseCmdLine() {
    ...
    if (selectedMatcher != null) {
        ...
        if (customConfig != null) {
            def configureMatcher = (customConfig =~ /configure:'(?<match>.*?)'/)
            if (configureMatcher.find()) {
                configureAppend += configureMatcher.group("match")
                customConfig -= ~/configure:'(?<match>.*?)'/
            }
            println "configure append: " + configureAppend
