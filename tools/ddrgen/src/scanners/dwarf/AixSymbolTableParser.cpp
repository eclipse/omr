 ...
 *				"accessibility","public"
 */
DDR_RC AixSymbolTableParser::additionalClassOptions(const string data, const double fileID, options_vect *classOptions)
{
	 ...
	 */
	if (FALSE == data.empty() && TRUE == regex_search(data, m, options_expression)) {
		...
		for (str_vect::iterator it = options.begin(); it != options.end(); ++it) {
			if (TRUE == regex_search(*it, m, option_attrs)) {
 ...
 */
string AixSymbolTableParser::extractAccessSpec(const string data, const int index)
{
	string ret = "";
 ...
 */
string AixSymbolTableParser::extractClassType(const string data)
{
	...
	if (FALSE == data.empty()) {
		...
		if (TRUE == regex_search(data, m, classStructure)) {
			ret = DECLARATION_TYPES[8];
		} else if (TRUE == regex_search(data, m, classUnion)) {
 ...
 */
double AixSymbolTableParser::extractTypeDefID(const string data, const int index)
{
 ...
 * @return typeID as double
 */
double AixSymbolTableParser::extractTypeID(const string data, const int index)
{
	stringstream ss;
	double ret = 0;
 ...
 */
double AixSymbolTableParser::generateChildTypeID(const double parentID, const double childOffset)
{
 ...
 */
double AixSymbolTableParser::generateInsertionKey(const double parentID, const double typeID)
{
 ...
 */
string AixSymbolTableParser::getBuiltInTypeDesc(const int typeID)
 {
	string ret = "";
 ...
 */
int AixSymbolTableParser::getBuiltInTypeSize(const int typeID)
{
	int ret = 0;
 ...
 */
DDR_RC AixSymbolTableParser::getNextFile(double *fileID)
{
	DDR_RC ret = DDR_RC_ERROR;
 ...
 */
string AixSymbolTableParser::getEntryName(const double insertionKey)
{
...
string AixSymbolTableParser::getInfoType(const string data, info_type_results *matchResults)
{
	...
	if (FALSE == data.empty()) {
		...
		if (TRUE == regex_search(data, m, typeDefExpression)) {
 ...
 */
bool AixSymbolTableParser::isBuiltIn(const string data, double *ID)
{
	bool ret = FALSE;
 ...
 */
bool AixSymbolTableParser::isBuiltIn(const int ID)
{
	return 0 > ID;
}
 ...
 */
bool AixSymbolTableParser::isCompilerGenerated(const string data)
{
 ...
 */
bool AixSymbolTableParser::isPassedByValue(const string data)
{
	bool ret = FALSE;
 ...
 */
DDR_RC AixSymbolTableParser::insertEntry(const double insetionKey, const Info object)
{
	DDR_RC ret = DDR_RC_ERROR;
 ...
 */
DDR_RC AixSymbolTableParser::insertFileEntry(const string fileName, const double fileID)
{
	DDR_RC ret = DDR_RC_ERROR;
 ...
 */
DDR_RC AixSymbolTableParser::insertRejectedEntry(const double insertionKey, Info *object)
{
	DDR_RC ret = DDR_RC_ERROR;
 ...
 */
int AixSymbolTableParser::isSigned(const int ID)
{
	int ret = 0;
	switch (ID) {
	case -1:
 ...
 */
bool AixSymbolTableParser::isValidField(const string data, Info *field)
{
	...
	if (TRUE == regex_search(data, m, expression)) {
		...
		if (FALSE == isBuiltIn(field_attrs[0], &(field->typeDef.typeID))) {
 ...
 */
bool AixSymbolTableParser::isValidFriendClass(const string data, Info *friendClass)
{
	bool ret = FALSE;
 ...
 */
bool AixSymbolTableParser::isValidNestedClass(const string data, Info *nestedClass)
{
	bool ret = FALSE;
	...
	if (1 == regex_search(data, m, expression)) {
 ...
 */
string AixSymbolTableParser::parseBound(const string data)
{
	string ret = "INVALID";
 ...
 */
DDR_RC AixSymbolTableParser::parseClassMembers(const string data, const double classID, const double fileID, double_vect *classMembers)
{
	DDR_RC ret = DDR_RC_ERROR;

	if (FALSE == data.empty() && 0 < classID && 0 < fileID) {
		...
		for (str_vect::iterator itt = preParsedMembers.begin(); itt != preParsedMembers.end(); ++itt) {
			if (!(*itt).empty() && *itt != "\n") {
				...
				if (TRUE == isValidDataMember(*itt, entry)) {
				...
				} else if (TRUE == isValidNestedClass(*itt, entry)) {
				...
				} else if (TRUE == isValidFriendClass(*itt, entry)) {
 ...
 * @return DDR_RC_OK if the data string was of correct format and DDR_RC_ERROR otherwise
 */
DDR_RC AixSymbolTableParser::parseFields(const string data, const double parentID, const double file_insertion_key, double_vect *memberIDs)
{
	DDR_RC ret = DDR_RC_ERROR;
	...
	if (FALSE == data.empty()) {
		...
		for (int index = 1; index <= tmp.size(); index++) {
			...
			Info *field = new Info();
			if (TRUE == isValidField(tmp[index - 1], field)) {
 ...
 * @return DDR_RC_OK if the data string was of correct format and DDR_RC_ERROR otherwise
 */
DDR_RC AixSymbolTableParser::parseVtblPointerInfo(const string data, options_vect *pointerInfo)
{
	DDR_RC ret = DDR_RC_ERROR;
 ...
 * @return DDR_RC_OK if the data string was of correct format and DDR_RC_ERROR otherwise
 */
DDR_RC AixSymbolTableParser::parseEnumData(const string enumeratorData, const double enumID, const double fileID, double_vect *enumeratorIDs)
{

	DDR_RC ret = DDR_RC_ERROR;
	if (FALSE == enumeratorData.empty() && 0 < enumID && 0 < fileID) {
...
DDR_RC AixSymbolTableParser::parseInfo(const string rawData,const double fileID, double *memberID)
{
	...
	if (FALSE == rawData.empty() && 0 < fileID) {
		...
		if (DECLARATION_TYPES[1] == info_type_result) { /* type def declaration */
			 ...
			 */
			if (FALSE == matchResults->members.empty()) {
			 ...
			 */
			if (FALSE == matchResults->members.empty()) {
				parseFields(matchResults->members, declData->typeID.ID, fileID, &declData->members);
			} else {
				ret = DDR_RC_OK;
			}
...
int AixSymbolTableParser::parseDumpOutput(const string data)
{
	...
	if (0 == _startNewFile && regex_search(data, m_s, fileSection)) {
		...
		if (TRUE == regex_search(data, m_s, fileInfoRow)) { /* Get file name */
			fileName = m_s.suffix();
		...
		if (TRUE == regex_search (data, m_s, sectionStart)) {
		...
		} else if (TRUE == regex_search (data, m_s, checkSymbol)) {
 ...
 */
string AixSymbolTableParser::stripLeading(const string str, const char chr)
{
	string ret = str;
	if (FALSE == ret.empty()){
		if (chr == str[0]){
 ...
 */
string AixSymbolTableParser::striptrailing(const string str, const char chr)
{
	string ret = str;
	if (FALSE == ret.empty()){
 ...
 */
double AixSymbolTableParser::shiftDecimal(const double value)
{
	double ret = value;
	if (ret >= 1) {
 ...
 */
double AixSymbolTableParser::toDouble(const string value)
{
	stringstream ss;
	double ret = 0;
	ss << value;
 ...
 */
int AixSymbolTableParser::toInt(const string value)
{
 ...
 */
string AixSymbolTableParser::toString(const double value)
{
	stringstream ss;
	ss << value;
	return ss.str();
 ...
 */
DDR_RC AixSymbolTableParser::updateEntry(data_map::iterator entry, Info update, bool overRideChecks)
{
	DDR_RC ret = DDR_RC_ERROR;

	if (FALSE == overRideChecks) {
		if (((0 == entry->second.name.compare(update.name))
			...
			if (FALSE == update.members.empty()) {
				entry->second.members = update.members;
			}

			if (FALSE == update.options.empty()) {
 ...
 */
string AixSymbolTableParser::beautifyVector(const vector<double> data, const string location,const int level)
{
...
string AixSymbolTableParser::beautifyVector(const str_vect data, const string location,const int level)
{
	string padding = getPadding(level);
	string output = "";
	string delimeter = " ---- ";
 ...
 */
string AixSymbolTableParser::beautifyInfo(const Info data, const double key, const bool overrideChildTag, const int level)
{
	string padding = "";
	...
	if ((TRUE == overrideChildTag) || (0 == data.typeID.isChild)) {
		...
		output += subLevelPadding + "declFile : " + data.declFile + "\n";
 ...
 */
void AixSymbolTableParser::printMapContents()
{
 ...
 */
string AixSymbolTableParser::beautifyMatchResults(const smatch m, const string location)
{
	string output = "";
	string outputDelimeter = "------------\n";
 ...
 */
string AixSymbolTableParser::beautifyOptionsVector(const options_vect data, const string location, const int level)
{
	string padding = getPadding(level);
	string output = "";
	string delimeter = " ---- ";
 ...
 */
string AixSymbolTableParser::beautifyInfoTypeResults(const info_type_results results, const string location)
{
	string output = "";
	string outputDelimeter = "------------\n";
